...
export function ListeLogements({
  statut,
  onReturn,
  onClickLogement,
}: ListeLogementsProps) {
  ...
  const handleUpdate = (logementUpdate: Logement) => {
    setListeLogements(
      listeLogements.map((logement) =>
        logement.id === logementUpdate.id ? logementUpdate : logement
      )
    );
  ...
  return (
    <div>
      {!souscription && user.user?.role.isOwner && !user.user.role.isAdmin && (
        <label className="label_info">
          Vous devez payer votre cotisation pour rendre actif vos logements !
        </label>
      )}
      {!logement && (
        <div>
          ...
          <div className="div_liste_horizontal">
            {statut && statut === "attenteValidation" && (
              <label className="label_info">
                Aucun logement en attente de validation
              </label>
            )}
            {listeLogements.map((logement) => (
              <LogementComponent
                key={logement.id}
                logement={logement}
                onClick={(logement) => handleClickLogement(logement)}
              />
            ))}
          </div>
        </div>
      ...
      {logement && (
        <LogementDetail
          logement={logement}
          onUpdate={(logement) => handleUpdate(logement)}
