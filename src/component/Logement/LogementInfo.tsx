...
import {
  getLogement,
  updateLogement,
  updateStatutLogement,
} from "../../request/requestLogement";
...
export function LogementInfo({
  ...
  useEffect(() => {
    if (listPhoto.length > 0) {
      setPhotoSrcMain(listPhoto[0].path);
    }
  }, [logement]);
  ...
  const handleAddPhoto = async (photo: File) => {
    ...
    if (photoCreated) {
      ...
      setPhotoSrcMain(photoCreated.path);
  ...
  const handleDelete = async (photoDelete: Photo) => {
    ...
    if (photoSrcMain === photoDelete.path) {
      if (
        listPhoto.filter((photo) => photo.id !== photoDelete.id).length === 0
      ) {
        setPhotoSrcMain("/icone/image.png");
      } else {
        setPhotoSrcMain(
          listPhoto.filter((photo) => photo.id !== photoDelete.id)[0].path
        );
      }
    }
  };
  ...
  return (
    <div>
      <form className="div_detail" onSubmit={(e) => handleSubmit(e)}>
            ...
            <div className="div_list_img_petite">
              {listPhoto.map((photo) => (
                <div key={photo.id}>
                  <img
                    src={photo.path}
                    onClick={() => setPhotoSrcMain(photo.path)}
                  ></img>
                  {isEdition && (
          ...
          <div className="div_logement_info">
            <div className="div_logement_info_content">
              ...
              <div className="div_info_value">
                ...
                <input
                  disabled={true}
                  value={typeBienString[logement.typeLogement]}
                ></input>
              </div>
              <div className="div_info_value">
                ...
                <select
                  ...
                  value={typeLocation}
                  onChange={(e) =>
                    setTypeLocation(e.target.value as TypeLocation)
                  }
                >
                  <option key={0} value={"entier"}>
                    {typeLocationString["entier"]}
                  </option>
                  <option key={1} value={"partiel"}>
                    {typeLocationString["partiel"]}
                  </option>
                </select>
              </div>
              <div className="div_info_value">
              ...
              {user.user?.role.isOwner && (
                <div className="div_info_value">
                  ...
                  <input
                    ...
                    value={
                      logement.isActif
                        ? statutLogementString[logement.statut]
                        : "Inactif"
                    }
                  ></input>
                </div>
        ...
        {user.user?.role.isOwner && (
          <div className="div_button_info_logement">
            {!isEdition && !user.user.role.isAdmin && (
              <button
                type="button"
                className="button"
            ...
            {user.user.role.isAdmin &&
              logement.statut === "attenteValidation" && (
                <button
                  type="button"
                  className="button"
      ...
      <PopupAddPhoto
        isOpen={isPopupOpen}
        onUpload={(photo) => handleAddPhoto(photo)}
